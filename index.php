...
<body>
		...
		<div id="block-sorting">
			...
			<li><a id="select-sort"><?php echo $sort_name; ?></a>
					<ul id="sorting-list">
						...
						<li><a href="index.php?sort=price-desc">От дорогим к дешевым</a></li>
						<li><a href="index.php?sort=popular">Популярный</a></li>
						<li><a href="index.php?sort=news">Новинки</a></li>
						<li><a href="index.php?sort=brand">От A до Я</a></li>
					</ul>
				</li>
			</ul>
		</div>
	...
	<?php
	  // Количество страниц и выводимого товара
		$num = 6; //Здесь указываем сколько хотим выводить товара
		$page = (int)$_GET['page']; //помешение значение page(1,2,3 и тд)
		
		$count = mysqli_query($link,"SELECT COUNT(*) FROM table_products WHERE visible= '1'");//COUNT (*)- подсчитывает количество товара
	...
	if($page + 5 <= $total) $page5rigth = '<li><a href ="index.php?page='.($page+5).'">'.($page+5).'</a></li>';
	if($page + 4 <= $total) $page4rigth = '<li><a href ="index.php?page='.($page+4).'">'.($page+4).'</a></li>';
	if($page + 3 <= $total) $page3rigth = '<li><a href ="index.php?page='.($page+3).'">'.($page+3).'</a></li>';
	if($page + 2 <= $total) $page2rigth = '<li><a href ="index.php?page='.($page+2).'">'.($page+2).'</a></li>';
	if($page + 1 <= $total) $page1rigth = '<li><a href ="index.php?page='.($page+1).'">'.($page+1).'</a></li>';
		...
		if($total > 1){
			echo'
				<div class = "pstrnav">
				<ul>';
			echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class ='sptr_active' href='index.php?page=".$page."'>".$page."</a></li>".$page1rigth.$page2rigth.$page3rigth.$page4rigth.$page5rigth.$strtotal.$pstr_next;
			echo '
				</ul>
				</div>
			';
