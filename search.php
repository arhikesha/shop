...
<head>
<meta charset="utf-8">
<title>Поиск -<? echo $sorting; ?> </title>
...
<body>
	...
	<?
		if(strlen ($search)>=2 && strlen ($search) < 96 ){
	
	?>

	<!--блок Сортировки grid -->
		<?
		echo '<div id="block-sorting">
			...
			<li><a id="select-sort">'. $sort_name.'</a>
					<ul id="sorting-list">
						...
						<li><a href="search.php?q='.$search.'&sort=price-desc">От дорогим к дешевым</a></li>
						<li><a href="search.php?q='.$search.'&sort=popular">Популярный</a></li>
						<li><a href=""search.php?q='.$search.'&sort=news">Новинки</a></li>
						<li><a href=""search.php?q='.$search.'&sort=brand">От A до Я</a></li>
					</ul>
				</li>
			</ul>
		</div>
	...
	<?php
	  // Количество страниц и выводимого товара
		$num = 4; //Здесь указываем сколько хотим выводить товара
		$page = (int)$_GET['page']; //помешение значение page(1,2,3 и тд)
		
		$count = mysqli_query($link,"SELECT COUNT(*) FROM table_products WHERE title LIKE '%$search%' AND visible= '1'");//COUNT (*)- подсчитывает количество товара
		...
		$result = mysqli_query($link,"SELECT * FROM table_products WHERE `title` LIKE '%$search%' AND visible= '1' ORDER BY $sorting $query_start_num ");
	...
	if($page !=$total)$pstr_next = '<li><a class="pstr-next" href="isearch.php?q='.$search.'&page='.($page+1).'">&lt;</a></li>';
	  
		/////Формируем ссылкии со страницами
	if($page - 5 > 0) $page5left = '<li><a href ="isearch.php?q='.$search.'&page='.($page-5).'">'.($page-5).'</a></li>';
	...
	if($page + 5 <= $total) $page5rigth = '<li><a href ="search.php?q='.$search.'&page='.($page+5).'">'.($page+5).'</a></li>';
	if($page + 4 <= $total) $page4rigth = '<li><a href ="search.php?q='.$search.'&page='.($page+4).'">'.($page+4).'</a></li>';
	if($page + 3 <= $total) $page3rigth = '<li><a href ="search.php?q='.$search.'&page='.($page+3).'">'.($page+3).'</a></li>';
	if($page + 2 <= $total) $page2rigth = '<li><a href ="search.php?q='.$search.'&page='.($page+2).'">'.($page+2).'</a></li>';
	if($page + 1 <= $total) $page1rigth = '<li><a href ="search.php?q='.$search.'&page='.($page+1).'">'.($page+1).'</a></li>';
		...
		if($total > 1){
			echo'
				<div class = "pstrnav">
				<ul>';
			echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class ='sptr_active' href='search.php?q=".$search."&page=".$page."'>".$page."</a></li>".$page1rigth.$page2rigth.$page3rigth.$page4rigth.$page5rigth.$strtotal.$pstr_next;
			echo '
				</ul>
				</div>
			';
		...
		echo '</ul>';
				...
				echo "<p>Нечего не найдено</p>";
			}
		}else {
			echo "<p>Поисковое значение должно быть от 2 и до 64 символов</p>";
		}
	?>
